...
import me.raining.gateway.common.utils.PropertiesUtils;
 ...
 * @author raining
 * @version 1.0.0
 * @description 网关核心配置加载类
 * 从五个地方加载配置，优先级高的会覆盖优先级低的
 * 优先级：运行参数 > jvm参数 > 环境变量 > 配置文件 > 配置对象对默认值（最低）
 */
@Slf4j
            ...
            try {
                ...
                if (inputStream != null) {
                    try {
                        ...
                        log.warn("load config file {} close error", CONFIG_FILE, e);
                    }
                }
            }
        }
