...
import me.raining.gateway.core.context.IContext;
 ...
 * @author raining
 * @version 1.0.0
 * @description 构建网关响应FullHttpResponse的辅助类
 */
public class ResponseHelper {
    ...
    public static FullHttpResponse getHttpResponse(ResponseCode responseCode) {
        LiteGatewayResponse gatewayResponse = LiteGatewayResponse.buildGatewayResponse(responseCode);
     ...
     */
    private static FullHttpResponse getHttpResponse(IContext ctx, LiteGatewayResponse gatewayResponse) {
        ByteBuf content;
    ...
    public static void writeResponse(IContext context) {
        ...
        if (context.isWritten()) {
            //	1：第一步构建响应对象，并写回数据
            FullHttpResponse httpResponse = ResponseHelper.getHttpResponse(context, (LiteGatewayResponse) context.getResponse());
